// api/polish.js — CORS + optional X-API-Key + strips markdown fence.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!text || typeof text !== "string") return res.status(400).json({ error:"Field 'text' is required" });

    const apiKey = req.headers["x-api-key"] || process.env.GEMINI_API_KEY;
    const prompt = `You are a writing assistant. Fix grammar, punctuation, and casing.
Keep meaning. Be concise. Return only the corrected text, no extra quotes or code fences.\n\nText:\n${text}`;
    ...
    let out = (data?.candidates?.[0]?.content?.parts?.map(p=>p.text).join("") || text).trim();
    out = out.replace(/^```[\s\S]*?\n/, "").replace(/```$/,"").trim(); // strip accidental fences
